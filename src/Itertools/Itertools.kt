 ...
 */
class PermutationReplaceIter<T>(val alphabet: List<T>, val length: Int) : Iterator<List<T>> {
 ...
 */
fun <T> permutationsWithReplacement(alphabet: List<T>, length: Int) = PermutationReplaceIter<T>(alphabet, length)

// overloaded for alphabet as String
fun permutationsWithReplacement(alphabet: String, length: Int) = permutationsWithReplacement(alphabet.map { it }, length)
